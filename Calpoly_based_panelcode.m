...
% Properties needed: E1, E2, G12, v12, a11, a22, rho, E1, E2,, G12, v12, a11, a22, F1t, F2t, F12, F1c, F2c, e1t, e2t, e12, e1c, e2c
...
% Sweep through core thickness and density
for k = 1:length(cthick)
    for kk = 1:length(pthick)
        ...
        % loop over each ply to integrate the ABD matriciesi
        for i = 1:n
            ...
            N_t = N_t + delT*Qxy*inv(T2)*a*(h(i+1) - h(i));
            M_t = M_t + delT*Qxy*inv(T2)*a*(h(i+1)^2 - h(i)^2);

            % Caalculate mass of the panel
        ...
        for i = 1:n
            ...
            for j = 1:2
                ...
                % Load alphas into an array
                a = [M(mi,6); M(mi,7); 0];
                e_thermal = delT*inv(T2)*a;
                % Form Qxy
                Qxy = inv(T1)*Q*R*T1*inv(R);
                % calculate distance from midsurface
                z = h(i-1+j);
                % transform back ino principle directions
                ...
                % FIF is indesx on fiber failure
                ...
                % Write total loval Strean for each ply on top and bottom into a matrix
